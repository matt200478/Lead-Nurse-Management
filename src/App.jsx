        ...
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50 print:hidden">
          <div className="bg-white rounded-xl shadow-xl p-6 w-[500px] max-w-[90vw] max-h-[90vh] overflow-y-auto relative">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setIsEditingTargets(false); setClinicToDelete(null); }} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>
            </div>
              ...
              {clinicTypes.map(clinic => (
                <div key={clinic} className="flex items-center justify-between gap-3 bg-slate-50 p-2 rounded-lg border border-slate-200">
                  ...
                  <div className="relative rounded overflow-hidden border border-slate-300 w-7 h-7 shrink-0 cursor-pointer focus-within:ring-2 focus-within:ring-blue-500 hover:scale-105 transition-transform">
                    <input type="color" value={clinicColors[clinic] || '#3b82f6'} onChange={(e) => handleUpdateTargetColor(clinic, e.target.value)} className="absolute -top-2 -left-2 w-12 h-12 cursor-pointer" />
                  </div>
                  <input type="number" min="0" className="w-16 p-1 border border-slate-300 rounded text-center text-sm outline-none focus:border-blue-500" value={targets[clinic]} onChange={(e) => handleUpdateTarget(clinic, e.target.value)} />
...
const INITIAL_TRAINING_STAFF = [
  { id: 1, name: 'Michelle Scotney', role: 'Nurse', records: {}, remarks: '' }
];
...
function TrainingMatrix() {
    ...
    const handleDeleteStaff = (id) => {
        updateDb('matrixStaff', matrixStaff.filter(s => s.id !== id));
    };
    ...
    return (
        <div className="flex-1 bg-slate-50 min-h-full font-sans text-slate-800 print:bg-white print:p-0">
            ...
            <div className="p-6 max-w-[1600px] mx-auto print:p-0 print:max-w-full">
                ...
                {/* Filters */}
                <div className="mb-4 flex gap-2 overflow-x-auto pb-2 print:hidden scrollbar-hide">
                ...
                <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-x-auto print:border-none print:shadow-none">
                    <table className="w-full text-left border-collapse text-sm">
                        ...
                        <tbody>
                            {filteredData.map((staff, idx) => (
                                <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors print:border-slate-300">
                    ...
                    {filteredData.length === 0 && (
                        <div className="p-8 text-center text-slate-500">
                            No staff members found matching your criteria. Click "Manage Staff" to add members.
                        </div>
                    )}
                </div>
            </div>
            ...
            {isManagingStaff && (
              <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50 print:hidden">
                <div className="bg-white rounded-xl shadow-xl w-[500px] max-w-[90vw] max-h-[90vh] flex flex-col relative">
                  <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                    <h3 className="text-lg font-bold">{editingStaff ? (editingStaff.id ? 'Edit Staff' : 'Add New Staff') : 'Manage Matrix Staff'}</h3>
                    <button onClick={() => { setIsManagingStaff(false); setEditingStaff(null); }} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>
                  </div>
                  <div className="p-6 overflow-y-auto">
                    {!editingStaff ? (
                      <div className="space-y-4">
                        <button onClick={() => setEditingStaff({ name: '', role: 'Nurse', records: {}, remarks: '' })} className="w-full py-2 flex items-center justify-center gap-2 bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 font-medium transition-colors border border-indigo-200">
                          ...
                          {matrixStaff.map(staff => (
                            <div key={staff.id} className="flex items-center justify-between p-3 border border-slate-200 rounded-lg bg-slate-50">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button onClick={() => handleDeleteStaff(staff.id)} className="p-1.5 text-slate-500 hover:text-red-600 rounded bg-white border border-slate-200 shadow-sm"><Trash2 className="w-4 h-4" /></button>
                              </div>
                            </div>
                          ))}
                        </div>
