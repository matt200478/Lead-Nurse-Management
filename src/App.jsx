...
import { 
  ...
  Download, Archive, BookOpen, Calculator
} from 'lucide-react';
...
// ============================================================================
// 4. ANNUAL LEAVE CALCULATOR COMPONENT
// ============================================================================
...
function AnnualLeaveCalculator() {
  ...
  const calculations = useMemo(() => {
    ...
    // 2. Pro Rata Multiplier
    const daysInLeaveYear = Math.ceil((leaveEndObj - leaveStartObj) / (1000 * 60 * 60 * 24)) + 1;
    
    // Constrain employment dates to within the leave year for calculation
    ...
    // 3. Entitlement Calculation (Hours)
    const standardDailyHours = standardFTEHours / 5; // Usually 7.5
            ...
            <div>
              ...
              <div className="flex gap-1 justify-between">
                {['M','T','W','T','F','S','S'].map((day, idx) => {
                  const dayIdx = idx === 6 ? 0 : idx + 1; // Map array to JS Date.getDay() (Sun=0)
                  return (
                    <button 
                      key={idx} 
...
// ============================================================================
// 5. MAIN APPLICATION SHELL & DASHBOARD
// ============================================================================
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div onClick={() => setView('training')} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 cursor-pointer hover:shadow-md hover:border-indigo-300 transition-all group relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm text-slate-500 mb-4 leading-relaxed">Central database to monitor staff qualifications, update completion dates, and track upcoming expiries.</p>
...
export default function App() {
  const [currentView, setCurrentView] = useState('dashboard'); // 'dashboard', 'rota', 'training', 'leave'
      ...
      <div className={`${isGlobalSidebarOpen ? 'w-64' : 'w-20'} transition-all duration-300 ease-in-out bg-slate-900 text-white shrink-0 flex flex-col print:hidden shadow-xl z-50 relative`}>
        ...
        <div className="flex-1 py-6 flex flex-col gap-2 px-4 overflow-hidden">
          ...
          <button 
            ...
            className={`flex items-center ${isGlobalSidebarOpen ? 'gap-3 px-4 py-3' : 'justify-center p-3'} rounded-xl font-medium transition-colors ${currentView === 'leave' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
